...
const ACCOUNT_COUNT: usize = 19;

/// Base seeds — one unique 32-byte array per named account.
/// Each scenario derives its own seeds by XOR-ing with a per-scenario byte.
const BASE_SEEDS: [[u8; 32]; ACCOUNT_COUNT] = [
    // SimpleA
    [
...
fn scenario_xor_byte(scenario: ScenarioName) -> u8 {
    match scenario {
        ...
        ScenarioName::DualRestart => 0xDD,
    }
}

fn derive_seed(scenario: ScenarioName, account: NamedAccount) -> [u8; 32] {
    let base = BASE_SEEDS[account.index()];
    let xor = scenario_xor_byte(scenario);
    let mut seed = [0u8; 32];
    for i in 0..32 {
        seed[i] = base[i] ^ xor;
    }
